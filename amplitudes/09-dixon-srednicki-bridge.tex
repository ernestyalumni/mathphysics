...
\begin{itemize}
  \item Srednicki uses the \emph{mostly-plus} metric $g_{\mu\nu}=\mathrm{diag}(-1,+1,+1,+1)$ (Srednicki Eq.~2.4).
  \item Dixon uses the \emph{mostly-minus} metric $g_{\mu\nu}=\mathrm{diag}(+1,-1,-1,-1)$ throughout arXiv:1310.5353.
  \item \textbf{Warning:} These are \emph{opposite} conventions. When translating between them, factors of $(-1)$ can appear in $p^2$, $s_{ij}$, and propagators. We flag these explicitly below.
...
\end{equation}
has $\det(k_{\alpha\dot\alpha})=0$ for $k^2=0$. Hence it has rank 1 and \emph{factorizes}:
\begin{equation}
...
In 2-component notation, using $\gamma_5 = \mathrm{diag}(-1,-1,+1,+1)$ (Srednicki's mostly-plus Weyl rep):
\begin{equation}
